...
int maxIntegerDigits(qreal top, qreal bottom)
{
    ...
    return static_cast<int>(std::floor(std::log10(maxAbs))) + 1;
}
}
...
QValidator::State DoubleInputValidator::validate(QString& inputStr, int& cursorPos) const
{
    ...
    if (inputStr.contains(validRegex)) {
        QRegularExpression invalidZeroRegex(QString("^\\-?0{2,%1}").arg(maxIntDigits) + decimalSep); // e.g. '-000,' or '-000.'
        ...
        if (inputStr.contains(invalidZeroRegex)
            ...
            state = Acceptable;
               ...
               || inputStr.contains(QRegularExpression(QString("^\\-?\\d{0,%1}" + decimalSep + "\\d{0,%2}$").arg(maxIntDigits).arg(m_decimal)))) {
        state = Intermediate;
    } else {
        cursorPos = 0;
        return Invalid;
